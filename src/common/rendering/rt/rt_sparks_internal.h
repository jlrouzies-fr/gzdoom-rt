...
// GENERATED by `py -3 tools/gen_caco_ball_recolor.py --ramp`. Do not hand-edit;
// re-run it if the recolour is retuned.
//
// THE BLUE HALF of 64CacodemonBall's impact fire, which burns half red and half
// blue -- and ONLY when the classic-recolour add-on is on, because that is when
// the ball itself has a blue fringe. See RT_CacoBlueFire().
//
// It is RT_FIRE_RED_RAMP above run through the recolour's own cold twin at full
// strength: the same transform that put the blue on the sprite's fringe, so the
// two halves of the fire are the two halves of the ball, and re-tuning the
// recolour moves both together.
//
// DERIVED rather than read off the art, which is the other way round from every
// ramp above and is the reason this comment is long. The recoloured burst's cool
// pixels are a TWO-DIMENSIONAL family -- source ramp entry crossed with fringe
// position -- so ten luminance quantiles through them walk in and out of violet
// (A037A7, 5E37CA, 812AA1, 4432C1 ...). Nobody would catch that on a five-frame
// spark; on a mark held on a wall for seconds it reads as the fire changing its
// mind.
constexpr uint32_t RT_FIRE_CACO_BLUE_RAMP[] = {
    0x3164FF, 0x305EFF, 0x2851FF, 0x2447FF, 0x2040FF,
    0x1B36E3, 0x12239E, 0x0D197E, 0x070E4E, 0x03052A,
};
constexpr int RT_FIRE_CACO_BLUE_RAMP_N = int( std::size( RT_FIRE_CACO_BLUE_RAMP ) );
...
enum class ArcFlavor : uint8_t
{
    ...
    FireRed,
    // THE CACODEMON'S, split back out of FireRed the moment its ball stopped
    // being the baron's ball in a different sprite.
    //
    // Its first ramp IS FireRed's, so with the classic-recolour add-on OFF this
    // row and the one above are the same flavour twice and nothing has moved.
    // What it adds is a SECOND ramp -- ArcStyle::ramp2 -- and a fire mark whose
    // style carries one burns half its flames on each. That is the only place in
    // this enum where a flavour is two colours, and it is still a colour, which
    // is what a flavour is.
    FireCaco,
    COUNT,
};
...
inline bool IsFireFlavor( ArcFlavor f )
{
    return f == ArcFlavor::FireOrange || f == ArcFlavor::FireViolet ||
           ...
           f == ArcFlavor::FireCaco;
}

// Is the Cacodemon's ball wearing its blue fringe right now?
//
// TRUE means d64r-caco-ball-recolor.pk3 is loaded, which the launcher passes
// only when the player ticked the classic-recolour add-on -- so this is the
// player's own choice, read back rather than pinned. rt_fire_caco_blue is the
// override: 1 follows the pk3, 0 forces the stock red fire, 2 forces the split
// on without it (the arm that shows the blue half against the orange ball).
//
// The pk3 lookup is cached: it cannot change without a restart, and this is
// asked once per fire mark per frame.
...
struct ArcStyle
{
    ...
    // A SECOND RAMP, and only ImpactFx::Fire reads it. Non-null means the mark's
    // flames alternate between the two -- half on each -- rather than all
    // cooling through one. Left null by every row that is one colour, which is
    // every row but the Cacodemon's, so the aggregate initialisers above stay
    // exactly as they were.
    const uint32_t* ramp2;
    int             ramp2N;
};
